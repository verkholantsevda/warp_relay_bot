...
# First time init
# sudo apt-get install jq wget wireguard-tools -y --fix-missing
...
sqlite3 "$DB" <<SQL
INSERT INTO warp_keys (user_id,country,config,created_at)
VALUES ($USER_ID,'$COUNTRY','$conf','$NOW');

INSERT INTO cfl_keys (key_id,id,token)
VALUES (
    (SELECT id FROM warp_keys ORDER BY id DESC LIMIT 1),
    '$id',
    '$token'
);
SQL
